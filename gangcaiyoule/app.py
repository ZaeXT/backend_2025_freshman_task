...
def logout():
    ...
    if response.status_code == 200:
        ...
        return "登出失败"
# 内容生成函数
def generate_content(prompt, history, model, conversation_id, is_role_play, role):
    ...
    if is_role_play:
        ...
        data["role"] = role_map.get(role, "")
    try :
        response = session.post(url, json=data)
    except Exception as e:
        history.append(("系统", f"生成请求失败:{e}"))
        return history
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=3):
            ...
            # 角色扮演模式选择
            role_mode = gr.Checkbox(label="启用角色扮演模式", value=False)
            ...
            role_discription = gr.Markdown("", visible=False)
            def toggle_role_ui(enable):
                return gr.update(visible=enable), gr.update(visible=False, value="")

            # 当用户勾选“启用角色扮演”时，显示角色下拉菜单
            role_mode.change(
                lambda enable: gr.update(visible=enable),
                inputs=role_mode,
                outputs=role_dropdown
            )
            # 当选择不同角色时，更新对应场景描述
            def update_role_description(role):
                if role == "小龙女":
                    desc = (
                        "**我是杨过，现在与我的师傅小龙女对话。**  \n"
                        "场景：杨过刚被小龙女勉强收留，他睡在寒冰床上觉得很冷但不敢出声。"
                    )
                elif role == "老师":
                    desc = (
                        "**你是一名严谨但关心学生的老师。**  \n"
                        "场景：学生在课堂上打瞌睡，你正在温和地提醒他。"
                    )
                elif role == "诸葛亮":
                    desc = (
                        "**你是睿智的诸葛亮。**  \n"
                ...
                return gr.update(value=desc, visible=True)
            role_dropdown.change(
                update_role_description,
                inputs=role_dropdown,
                outputs=role_discription
            )
    ...
    generate_button.click(
        generate_content,
        inputs=[prompt_input, chatbot, model_dropdown, conv_list, role_mode, role_dropdown],
        outputs=chatbot
    )
